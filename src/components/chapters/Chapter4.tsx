...
export function Chapter4({ onNext }: { onNext: () => void }) {
  ...
  const photoVariants = {
    ...
    visible: (custom: any) => ({
      ...
      transition: { delay: skip ? 0.1 : custom.delay, duration: skip ? 0.5 : 1.5, type: "spring", bounce: 0.3 }
    })
  };
  ...
  return (
    <div 
      ...
      onClick={() => { if (selectedPhoto === null) setSkip(true); }}
    >
      <BackgroundPhoto src={CONFIG.photos[3]} opacity={0.25} position="top" />

      <motion.h1 custom={1} initial="hidden" animate="visible" variants={textVariants} className="text-3xl md:text-5xl font-heading text-gradient text-center mb-8">
        Some Memories Don't Need Words.
      </motion.h1>
      
      <div className="text-center space-y-8 mb-12">
        <motion.p custom={3.5} initial="hidden" animate="visible" variants={textVariants} className="text-lg md:text-xl font-light text-white/90">
          Kuch photos sirf photos nahi hoti...<br/>
          unhe dekhte hi ek poora moment wapas aa jata hai.
        </motion.p>
        <motion.p custom={7} initial="hidden" animate="visible" variants={textVariants} className="text-lg md:text-xl font-light text-white/90">
          Aur tumhari kuch tasveerein... bas dekhne ke liye nahi hain.
        </motion.p>
        <motion.p custom={10.5} initial="hidden" animate="visible" variants={textVariants} className="text-xl md:text-2xl font-heading text-pink-200 italic">
      ...
      <div className="relative w-full h-[50vh] md:h-[60vh] flex items-center justify-center mt-8">
        {collagePhotos.map((photo, i) => {
          ...
          return (
            <motion.div
              key={i}
              custom={{ delay: 14 + i * 1.5, x: xOffset, y: yOffset, rotate: rotation }}
              initial="hidden"
              animate="visible"
      ...
      <motion.div custom={22} initial="hidden" animate="visible" variants={textVariants} className="pt-20 w-full flex justify-center pb-8 z-50">
        <button 
